...
export function Footer({ siteContent }: { siteContent: SiteContent }) {
  ...
  return (
    <footer
      className='bg-brand-black text-brand-cream relative mt-20 overflow-hidden'
      role='contentinfo'
    >
      {/* Decorative top gradient line */}
      <motion.div
        className='from-brand-orange via-brand-gold to-brand-orange absolute top-0 right-0 left-0 h-1 bg-gradient-to-r'
        initial={{ scaleX: 0 }}
      ...
      {/* Static brand glow — was an infinite scale/opacity blur on a 600px element,
          which repaints a large area every frame. A fixed tint reads the same at rest. */}
      <div className='bg-brand-gold/5 pointer-events-none absolute top-1/2 left-1/2 h-[600px] w-[600px] -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl' />
      ...
      <div className='relative mx-auto max-w-7xl px-4 py-12 sm:py-16' ref={ref}>
        ...
        >
          ...
          >
            ...
            <div className='text-brand-sand/80 space-y-2 text-sm'>
              ...
              >
                <span className='text-brand-sand font-medium'>Location:</span>{' '}
                <a
              ...
              >
                <span className='text-brand-sand font-medium'>Business:</span>{' '}
          ...
          {[
            ...
            <motion.div
              ...
              variants={{
                hidden: { opacity: 0, y: 30 },
                visible: { opacity: 1, y: 0, transition: { duration: 0.6, delay: colIndex * 0.1 } }
              }}
            >
              ...
              <ul className='space-y-2.5'>
                {column.links.map((link, i) => (
                  <motion.li
                    ...
                    animate={inView ? { opacity: 1, x: 0 } : {}}
                    transition={{ delay: 0.3 + colIndex * 0.05 + i * 0.03 }}
                  >
                    <Link
                      href={link.href}
                      className='text-brand-sand/80 text-sm transition-all hover:pl-1 hover:text-white'
                    >
                      {link.label}
                    </Link>
                  </motion.li>
        ...
        {/* Bottom bar */}
        <motion.div
          className='border-brand-sand/20 mt-12 border-t pt-8'
          initial={{ opacity: 0 }}
          animate={inView ? { opacity: 1 } : {}}
          transition={{ delay: 0.6 }}
        >
          <div className='flex flex-col items-center justify-between gap-4 sm:flex-row'>
            <p className='text-brand-sand/60 text-center text-xs sm:text-left'>
